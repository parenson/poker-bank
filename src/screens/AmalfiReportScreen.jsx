...
export default function AmalfiReportScreen({ onBack }) {
  ...
  useEffect(() => {
    const source = state.savedGames.length > 0 ? Promise.resolve(state.savedGames) : loadGames()
    source.then(games => {
  ...
  const summaries = useMemo(() => {
    ...
    for (const g of yearGames) {
      for (const r of g.results) {
        ...
        if (!byPlayer[key]) {
          byPlayer[key] = {
            name: r.name, initials: r.initials, venmoHandle: r.venmoHandle, lines: []
          }
        }
        ...
        <>
          ...
          <Card>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <span style={{ fontWeight: 600, fontSize: 14, color: 'var(--cream-dim)' }}>Year</span>
              <div style={{ display: 'flex', gap: 8, flex: 1 }}>
                {years.map(y => (
                  <button
                    key={y}
                    onClick={() => { setSelectedYear(y); setExpandedPlayers(new Set()) }}
                    style={{
          ...
          {summaries.length > 0 && (
            <Card title={`${selectedYear} Season Summary`}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 8 }}>
                ...
                <MiniStat label="Total Buy-ins" value={fmt(grandTotals.buyIns)} small />
                <MiniStat label="Total Cash Out" value={fmt(grandTotals.cashOut)} small />
              </div>
            </Card>
          )}
                    ...
                    >
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 12, color: 'var(--cream-dim)' }}>
                          {p.lines.length} game{p.lines.length !== 1 ? 's' : ''} · Buy-ins {fmt(p.totalBuyIns)} · Out {fmt(p.totalCashOut)}
                        </div>
                      </div>
                      <div style={{ textAlign: 'right', flexShrink: 0 }}>
                        <div style={{ fontFamily: 'Cinzel, serif', fontSize: 18, fontWeight: 700, color }}>
                          {fmtSigned(p.totalNet)}
                        </div>
                    ...
                    {expanded && (
                      <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', padding: '4px 0 8px' }}>
                        ...
                        <div style={{
                          ...
                          padding: '4px 13px 6px', gap: 4,
                        }}>
                          <div style={{ fontSize: 11, color: 'var(--cream-dim)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Game</div>
                          <div style={{ fontSize: 11, color: 'var(--cream-dim)', fontWeight: 600, textAlign: 'right', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Buy-in</div>
                          <div style={{ fontSize: 11, color: 'var(--cream-dim)', fontWeight: 600, textAlign: 'right', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Cash Out</div>
                          <div style={{ fontSize: 11, color: 'var(--cream-dim)', fontWeight: 600, textAlign: 'right', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Net</div>
                        </div>
                        ...
                        {p.lines.map((line, li) => {
                          ...
                          return (
                            <div key={li} style={{
                              ...
                              <div style={{ fontSize: 13, color: 'var(--cream)', textAlign: 'right' }}>
                                ${line.buyInTotal.toFixed(0)}
                              </div>
                              <div style={{ fontSize: 13, color: 'var(--cream)', textAlign: 'right' }}>
                                ${line.cashOut.toFixed(0)}
                              </div>
                        ...
                        <div style={{
                          ...
                          <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--cream)', textAlign: 'right' }}>
                            ${p.totalBuyIns.toFixed(0)}
                          </div>
                          <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--cream)', textAlign: 'right' }}>
                            ${p.totalCashOut.toFixed(0)}
                          </div>
...
function MiniStat({ label, value, small }) {
  return (
    <div style={{
      ...
      padding: '10px 8px', border: '1px solid rgba(201,168,76,0.1)',
      textAlign: 'center',
    }}>
      <div style={{ fontSize: 10, color: 'var(--cream-dim)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 3 }}>
        {label}
      </div>
      <div style={{
        fontFamily: small ? 'DM Sans, sans-serif' : 'Cinzel, serif',
        fontSize: small ? 13 : 18, fontWeight: 600, color: 'var(--gold-light)',
      }}>
        {value}
      </div>
    </div>
