...
export default function ResultsScreen() {
  ...
  const isAmalfi = setup.gameType === 'amalfi'
  ...
  const handleNewGame = () => dispatch({ type: 'RESET_GAME' })
    ...
    <Screen>
      ...
      <Card title="Game Summary">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          ...
          {!isAmalfi && <StatCard label="Dealer Tips" value={fmt(totalTipFromWinners)} />}
          {!isAmalfi && <StatCard label="Dealer Total" value={fmt(finalDealerTake)} highlight />}
        </div>
      </Card>

      <Card title="Standings">
        {sorted.map((s, i) => (
          <PlayerResult key={s.player.id} settlement={s} rank={i + 1} tipPct={tipPct} isAmalfi={isAmalfi} />
        ))}
      </Card>
      ...
      {winnerPayments.length > 0 && (
        <Card title={`${setup.bankerName || 'Banker'} → Winners`}>
          ...
          {winnerPayments.map((p, i) => (
            <PaymentRow key={i} payment={p} note="Poker winnings" />
          ))}
        </Card>
      )}

      {!isAmalfi && dealerPayment && (
...
function PlayerResult({ settlement: s, rank, tipPct, isAmalfi }) {
    ...
    <div style={{
      ...
      >
        <div style={{
          ...
          fontSize: 11, fontWeight: 700, color: 'var(--gold-dim)', flexShrink: 0,
        }}>
          {rank}
        </div>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--cream-dim)' }}>
            In {fmt(s.potIn + (isAmalfi ? 0 : s.upfrontDealer))} · Out {fmt(s.player.cashOut || 0)}
          </div>
        </div>
        <div style={{ textAlign: 'right', flexShrink: 0 }}>
          <div style={{ fontFamily: 'Cinzel, serif', fontSize: 18, fontWeight: 700, color }}>{fmtSigned(net)}</div>
      ...
      {open && (
        <div style={{ padding: '0 14px 12px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
          <DetailRow label="Buy-ins + rebuys" value={fmt(s.potIn)} />
          {!isAmalfi && <DetailRow label="Upfront dealer fee" value={`− ${fmt(s.upfrontDealer)}`} />}
          ...
          {!isAmalfi && s.netBeforeTip > 0 && s.netBeforeTip < 20 && (
...
function DetailRow({ label, value, bold, color }) {
  return (
    <div style={{
      ...
      <span style={{ fontSize: 13, color: 'var(--cream-dim)', flex: 1 }}>{label}</span>
      <span style={{ fontSize: 13, fontWeight: bold ? 700 : 500, color: color || 'var(--cream)', flexShrink: 0 }}>
        {value}
      </span>
    </div>
  )
...
function PaymentRow({ payment: p, note }) {
    ...
    <div style={{
      ...
      <div style={{ flex: 1 }}>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {toHandle && (
            <VenmoBtn txn="pay" handle={toHandle} amount={p.amount} note={note}
              label={`Pay @${toHandle}`} variant="pay" />
          )}
          {fromHandle && (
            <VenmoBtn txn="charge" handle={fromHandle} amount={p.amount} note={note}
              label={`Request @${fromHandle}`} variant="request" />
          )}
        </div>
      </div>
      <div style={{
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
        ...
        <div style={{ fontFamily: 'Cinzel, serif', fontSize: 22, fontWeight: 700, color: 'var(--success)' }}>
          +{fmt(total)}
        </div>
      </div>
      <Divider />
      <DetailRow label={`Upfront fees (${playerCount} × ${fmt(upfront / playerCount)})`} value={fmt(upfront)} />
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>
        {dealerHandle && (
          <VenmoBtn txn="pay" handle={dealerHandle} amount={total}
            note="Poker dealer payout" label={`Pay dealer @${dealerHandle}`} variant="pay" />
        )}
        {bankerHandle && (
          <VenmoBtn txn="charge" handle={bankerHandle} amount={total}
            note="Poker dealer payout" label={`Request from @${bankerHandle}`} variant="request" />
        )}
      </div>
    </div>
  )
